...
namespace Z.Frame
{
	public class UIManager : IOnUpdate
	{
		//主界面名字，用来特殊操作
		public const string MainPanelName = "MainPanel";
		public GameObject Bg;//黑色遮罩
		public GameObject Root;//根物体
		public List<UIPanelInfo> PlanesList = new List<UIPanelInfo>();  //所有界面列表

		private Queue<IPanelCMD> PanelCmds = new Queue<IPanelCMD>();  //指令队列
		...
		public void InitBG()
		{
			...
			if (obj == null)
			{
				Debug.LogError("Cannot find BlackBGPanel : Data/UI/BlackBGPanel");
				return;
			}
		...
		public void InitRoot()
		{
			...
			if (obj == null)
			{
				Debug.LogError("Cannot find UI Root : Data/UI/UI");
				return;
			}
		...
		public void OpenPanel(object panel, bool isCloseLast = true, bool needBg = false)
		{
		...
		{
			if (this.PanelCmds.Count > 0)
			{
				var cmd = this.PanelCmds.Dequeue();
				cmd.Excute(this);
				return 0;
			}
			else
	...
	public class UIPanelInfo : IPanel
	{
		...
		public void Show(UIManager uim)
		{
			...
			if (NeedBg)
			{
				uim.Bg.transform.SetSiblingIndex(uim.Root.transform.childCount -2);
		...
		public void Hide(UIManager uim)
		{
			...
			if (NeedBg)
			{
				uim.Bg.transform.SetAsFirstSibling();
	...
	class OpenPanelCmd : IPanelCMD
	{
		...
		public void Excute(UIManager uim)
		{
			...
			//播放音效
			//播放效果(让主界面直接生成)
			if(info.Name != UIManager.MainPanelName)
